...
export class Recorder {
  ...
  static ConnectEnableWorklet = false;
  ...
  static Destroy = function () {
    console.log(RecTxt + " Destroy");
    Disconnect(); //断开可能存在的全局Stream、资源

    for (let k in DestroyList) {
      DestroyList[k]();
    }
  };
  //登记一个需要销毁全局资源的处理方法
  static BindDestroy = function (key, call) {
  ...
  static PowerLevel = function (pcmAbsSum, pcmLength) {
  ...
  /*计算音量，单位dBFS（满刻度相对电平）
		...
		*/
  static PowerDBFS = function (maxSample) {
    let val = Math.max(0.1, maxSample || 0),
      Pref = 0x7fff;
  ...
  static Sync = { /*open*/ O: 9, /*close*/ C: 9 };

  //流量统计用1像素图片地址，设置为空将不参与统计
  static TrafficImgUrl = "//ia.51.la/go1?id=20469973&pvFlag=1";
  static Traffic = function (report) {
    report = report ? "/" + RecTxt + "/Report/" + report : "";
    let imgUrl = Recorder.TrafficImgUrl;
    if (imgUrl) {
      let data = Recorder.Traffic;
      let m = /^(https?:..[^\/#]*\/?)[^#]*/i.exec(location.href) || [];
      let host = m[1] || "http://file/";
      let idf = (m[0] || host) + report;
...
let RecTxt = "Recorder";
let getUserMediaTxt = "getUserMedia";
let srcSampleRateTxt = "srcSampleRate";
let sampleRateTxt = "sampleRate";
let CatchTxt = "catch";
let DestroyList = {};
...
let Connect = function (streamStore, isUserMedia) {
  let bufferSize = streamStore.BufferSize || Recorder.BufferSize;

  let ctx = Recorder.Ctx,
    stream = streamStore.Stream;
  let mediaConn = function (node) {
    let media = (stream._m = ctx.createMediaStreamSource(stream));
    let ctxDest = ctx.destination,
  ...
  let scriptProcessor = "ScriptProcessor"; //一堆字符串名字，有利于压缩js
  let audioWorklet = "audioWorklet";
  let recAudioWorklet = RecTxt + " " + audioWorklet;
  let RecProc = "RecProc";
  let MediaRecorderTxt = "MediaRecorder";
  let MRWebMPCM = MediaRecorderTxt + ".WebM.PCM";

  //===================连接方式三=========================
  //古董级别的 ScriptProcessor 处理，目前所有浏览器均兼容，虽然是过时的方法，但更稳健，移动端性能比AudioWorklet强
  let oldFn = ctx.createScriptProcessor || ctx.createJavaScriptNode;
  let oldIsBest =
    "。由于" +
    audioWorklet +
  ...
  let oldScript = function () {
    ...
    let process = (stream._p = oldFn.call(ctx, bufferSize, 1, 1)); //单声道，省的数据处理复杂
    mediaConn(process);

    let _DsetTxt = "_D220626",
      _Dset = Recorder[_DsetTxt];
    if (_Dset) console.log("Use " + RecTxt + "." + _DsetTxt, 3);
    process.onaudioprocess = function (e) {
      let arr = e.inputBuffer.getChannelData(0);
      if (_Dset) {
        //临时调试用的参数，未来会被删除
  ...
  let connWorklet = function () {
    ...
    let clazzUrl = function () {
      ...
      let clazz = "class " + RecProc + " extends AudioWorkletProcessor{";
      clazz +=
        "constructor " +
        xf(function (option) {
          // DEL_super(option);
      ...
      clazz +=
        ...
        xf(function (input, b, c) {
          ...
          if (input.length) {
            ...
            let len = ~~(pos / bufferSize) * bufferSize;
            if (len) {
              this.port.postMessage({ val: buffer.slice(0, len) });

              let more = buffer.subarray(len, pos);
    ...
    let nodeAlive = (stream._na = function () {
    ...
    let createNode = function () {
      if (!awNext()) return;
      let node = (stream._n = new AwNode(ctx, RecProc, {
      ...
      node.port.onmessage = function (e) {
        ...
        if (awNext()) {
          ...
          console.log(audioWorklet + "多余回调", 3);
        }
      };
      console.log(
        "Connect采用" +
    ...
    ctx.resume()[calls && "finally"](function () {
      ...
      ctx[audioWorklet]
        ...
        [CatchTxt](function (e) {
          ...
          console.log(audioWorklet + ".addModule失败", 1, e);
          awNext() && oldScript();
        });
    });
  };
  ...
  let connWebM = function () {
    ...
    let mrAlive = (stream._ra = function () {
    ...
    mr[onData] = function (e) {
      ...
      reader.onloadend = function () {
        if (mrNext()) {
          let f32arr = WebM_Extract(new Uint8Array(reader.result), webmData);
          if (!f32arr) return;
          if (f32arr == -1) {
            //无法提取，立即降级
            connWorklet();
          ...
          console.log(MediaRecorderTxt + "多余回调", 3);
        }
      };
...
let _Disconn_n = function (stream) {
  stream._na = null;
  if (stream._n) {
...
let _Disconn_r = function (stream) {
  stream._ra = null;
  if (stream._r) {
    stream._r.stop();
    stream._r = null;
...
let Disconnect = function (streamStore) {
  streamStore = streamStore || Recorder;
  let isGlobal = streamStore == Recorder;

  let stream = streamStore.Stream;
  if (stream) {
    if (stream._m) {
...
initFn.prototype = {
  ...
  open: function (True, False) {
    ...
    True = True || NOOP;
    let failCall = function (errMsg, isUserNotAllow) {
    ...
    let ok = function () {
      console.log("open ok id:" + This.id);
      True();
    ...
    //环境配置检查
    let checkMsg = This.envCheck({ envName: "H5", canProcess: true });
    if (checkMsg) {
    ...
    //***********打开麦克风得到全局的音频流************
    let codeFail = function (code, msg) {
      try {
        //跨域的优先检测一下
        window.top.a;
      } catch (e) {
    ...
    let f1 = function (stream) {
      ...
      setTimeout(function () {
        ...
        if (Recorder.IsOpen()) {
          if (oldStream) console.log("发现同时多次调用open", 1);
    ...
    let f2 = function (e) {
      let code = e.name || e.message || e.code + ":" + e;
      console.log("请求录音权限错误", 1, e);
    ...
    let trackSet2 = This.set.audioTrackSet;
    for (let k in trackSet2) trackSet[k] = trackSet2[k];
    trackSet.sampleRate = Recorder.Ctx.sampleRate; //必须指明采样率，不然手机上MediaRecorder采样率16k
    let pro=null;
    try {
      pro = Recorder.Scope[getUserMediaTxt]({ audio: trackSet }, f1, f2);
    } catch (e) {
      //不能设置trackSet就算了
      console.log(getUserMediaTxt, 3, e);
  ...
  envStart: function (mockEnvInfo, sampleRate) {
    ...
    if (This[set.type + "_start"]) {
      let engineCtx = (This.engineCtx = This[set.type + "_start"](set));
      if (engineCtx) {
        engineCtx.pcmDatas = [];
        engineCtx.pcmSize = 0;
      }
  ...
  envIn: function (pcm, sum) {
    ...
    let bufferSampleRate = This[srcSampleRateTxt];
    let size = pcm.length;
    let powerLevel = Recorder.PowerLevel(sum, size);
    ...
    let bufferFirstIdxThis = bufferFirstIdx;
    ...
    let envInFixTs = This.envInFixTs;
    ...
    if (engineCtx) {
      //转换成set的采样率
      let chunkInfo = Recorder.SampleData(
        buffers,
        bufferSampleRate,
        set[sampleRateTxt],
        engineCtx.chunkInfo
    ...
    try {
      ...
      //此错误显示不要用console.log，这样控制台内相同内容不会重复打印
      console.error(procTxt + "回调出错是不允许的，需保证不会抛异常", e);
    }

    let slowT = Date.now() - now;
    if (slowT > 10 && This.envInFirst - now > 1000) {
      //1秒后开始onProcess性能监测
      console.log(procTxt + "低性能，耗时" + slowT + "ms", 3);
    }
    ...
    if (asyncBegin === true) {
      ...
      if (hasClear) {
        console.log("未进入异步前不能清除buffers", 3);
      } else {
  ...
  start: function () {
    ...
    if (ctx.state == "suspended") {
      let tag = "AudioContext resume: ";
      console.log(tag + "wait...");
      ctx
        .resume()
        .then(function () {
          console.log(tag + ctx.state);
          end();
        })
        [CatchTxt](function (e) {
          //比较少见，可能对录音没有影响
          console.log(tag + ctx.state + " 可能无法录音：" + e.message, 1, e);
          end();
        });
    } else {
      end();
  ...
  resume: function () {
    ...
    if (This.state) {
      ...
      let stream = This._streamStore().Stream;
  ...
  stop: function (True, False, autoClose) {
    let This = this,
      set = This.set,
      t1;
    let envInMS = This.envInLast - This.envInFirst,
    ...
    let res = chunk.data;
    let duration = Math.round((res.length / set[sampleRateTxt]) * 1000);
...
let WebM_Extract = function (inBytes, scope) {
    ...
    while (position[0] < bytes.length) {
      ...
      if (BytesEq(eid0, [0x16, 0x54, 0xae, 0x6b])) {
        while (pos0[0] < bytes0.length) {
          let eid1 = readMatroskaVInt(bytes0, pos0);
          let bytes1 = readMatroskaBlock(bytes0, pos0);
          let pos1 = [0],
            track = { channels: 0, sampleRate: 0 };
          if (BytesEq(eid1, [0xae])) {
            //TrackEntry
            while (pos1[0] < bytes1.length) {
              let eid2 = readMatroskaVInt(bytes1, pos1);
              let bytes2 = readMatroskaBlock(bytes1, pos1);
              let pos2 = [0];
              if (BytesEq(eid2, [0xd7])) {
                //Track Number
                let val = BytesInt(bytes2);
                track.number = val;
                tracks[val] = track;
                ...
                while (pos2[0] < bytes2.length) {
                  //循环读取 Audio 属性
                  let eid3 = readMatroskaVInt(bytes2, pos2);
                  let bytes3 = readMatroskaBlock(bytes2, pos2);
                  //采样率、位数、声道数
                  if (BytesEq(eid3, [0xb5])) {
                    ...
                    else console.log("WebM Track !Float", 1, bytes3);
        ...
        scope._ht = 1;
        console.log("WebM Tracks", tracks);
        endPos();
        break;
      }
    }
  ...
  if (
    ...
    if (!scope.bad) console.log("WebM Track非预期", 3, scope);
    scope.bad = 1;
    return -1;
  }
  ...
  while (position[0] < bytes.length) {
    ...
    if (BytesEq(eid1, [0xa3])) {
      ...
      if (!track) {
        //不可能没有，数据出错？
        console.log("WebM !Track" + trackNo, 1, tracks);
      } else if (track.idx === 0) {
...
//两个字节数组内容是否相同
let BytesEq = function (bytes1, bytes2) {
...
//字节数组BE转成int数字
let BytesInt = function (bytes) {
  let s = ""; //0-8字节，js位运算只支持4字节
...
//读取一个可变长数值字节数组
let readMatroskaVInt = function (arr, pos, trim) {
  let i = pos[0];
...
//读取一个自带长度的内容字节数组
let readMatroskaBlock = function (arr, pos) {
  let lenVal = readMatroskaVInt(arr, pos, 1);
  if (!lenVal) return;
